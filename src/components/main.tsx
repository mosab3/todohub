        ...
        <div className="mb-3" key={index}>
          <div className="p-3 rounded-pill bg-info bg-opacity-10">
            <div className="position-relative">
              <div className="position-absolute top-50 start-0 translate-middle-y">{value.text}</div>
              <div className="position-absolute top-50 end-0 translate-middle-y">
                <span className="badge rounded-pill text-bg-info bg-opacity-75">
                  status:{' '}
                  <span className="badge rounded-pill text-bg-light bg-opacity-75">
                    {value.checked ? 'Completed' : 'Uncompleted'}
                  </span>
                </span>
                {todo.some((obj) => obj.text === value.text) && (
                  <span className="badge rounded-pill text-bg-danger bg-opacity-75">Duplicated</span>
                )}
              </div>
            </div>
          </div>
    ...
    if (shareType === ShareType.SEND) {
      return (
        <div>
          <div className="vstack gap-2 mx-auto">
            <div>{generateQR()}</div>
            <div>
              <button type="button" className="btn btn-primary" onClick={() => setShareType(null)}>
                Back
              </button>
            </div>
          </div>
      ...
      return (
        <div className="hstack gap-2 justify-content-center">
          <div className="p-2">
            <button type="button" className="btn btn-primary" onClick={() => setShareType(ShareType.SEND)}>
              Sender
            </button>
          </div>
          ...
          <div className="p-2">
            <button
              ...
              className="btn btn-primary"
              onClick={() => {
                setShareType(ShareType.RECEIVE)
                setResultObjs([])
              }}
            >
              Receiver
...
export function ToasterComponent() {
  ...
  return (
    <Toaster
      toastOptions={{ className: `${darkMode === 'light' ? 'bg-light text-dark' : 'bg-dark text-light'}` }}
    />
  )
}
